...
#define VERBOSE \
    1  // verboseness level: 0 silent, 1 normal (report result only), 2 verbose
#define IFVERB(x)       \
    if (VERBOSE >= 1) { \
        x;              \
    }
...
int main(void) {
    ...
    IFVERB2(for (vector<string>::iterator it = Prog.begin(); it != Prog.end();
                 ++it) cerr
                << "  " << *it << endl;);
    IFVERB2(cerr << "-----" << endl;);
    ...
    IP = SC = 0;
    while (1) {
        IFVERB2(cerr << "  ip=" << IP << " command=" << Prog[IP]
                     << " steps=" << SC << endl;);
        switch (Prog[IP][0]) {
            ...
            case 'P':  // print
                ...
                break;
            case 'Q':
                cerr << "Terminated correctly after " << SC << " steps."
                     << endl;
                return 0;
                break;  // quit
            case 'Z':   // jump if zero
                if (Prog[IP].length() == 1)
                    ERROR("Command missing a variable.");
                if ((Prog[IP][1] < 'a') || (Prog[IP][1] > 'z'))
                    ERROR("Incorrect variable name.");
                ...
                break;
            case ':':
                break;  // declaration of a label
            case '+':
                x = QGET();
                y = QGET();
                Q.push_back((x + y) % 65536);
                break;  // addition
            case '-':
                x = QGET();
                y = QGET();
                Q.push_back((65536 + x - y) % 65536);
                break;  // subtraction
            case '*':
                x = QGET();
                y = QGET();
            ...
            case '<':  // put from a variable
                ...
                break;
            default:
                Q.push_back(atoi(Prog[IP].c_str()) % 65536);
                break;  // convert to a number && push it
        }
        IP++;
        SC++;
