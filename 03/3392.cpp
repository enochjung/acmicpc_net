...
private:
		...
		void update(int s, int e, int a, int b, int idx, int value) {
			...
			if (s<=a && b<=e) {
				...
				if (idx >= size)
					...
					tree[idx].value = tree[idx].count>=1? tree[idx].gap : tree[idx*2].value+tree[idx*2+1].value;
				return;
			}
	...
	std::vector<int> compress_with(const std::function<std::pair<int,int>(const rectangle &r)> op) const {
		std::vector<int> pos;
	...
	std::vector<query> get_query(const std::vector<int> &xpos, const std::vector<int> &ypos) const {
		...
		std::sort(queries.begin(), queries.end(), [](const query &lhs, const query &rhs) -> bool {
			return lhs.x < rhs.x;
		});
...
public:
	...
	long long get_area() {
		std::vector<int> xpos = compress_with([](const rectangle &r) {
			return std::pair(r.x1, r.x2);
		});
		std::vector<int> ypos = compress_with([](const rectangle &r) {
			return std::pair(r.y1, r.y2);
		});
